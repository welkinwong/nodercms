    ...
    function (categories, callback) {
      async.map(categories, function (category, callback) {
        if (category.node) {
          ...
          contents.find({ category: category._id, status: 'pushed' })
            .sort('-date')
            .limit(50)
            .select('category title alias user date reading thumbnail')
            .populate('category', 'name path')
  ...
  async.waterfall([
    ...
    function (columns, callback) {
      async.map(columns, function (column, callback) {
        contents.find({ category: column._id, status: 'pushed' })
          .sort('-date')
          .limit(50)
...
exports.column = function (options, callback) {
  ...
  contentsService.list({ _id: _id, status: 'pushed', pageSize: pageSize, currentPage: currentPage }, function (err, result) {
    if (err) return callback(err);
